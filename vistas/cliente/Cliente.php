...
// 6) Mostrar libros locales (view=tienda)
    } elseif ($view === 'tienda') {
        require_once __DIR__ . '/../../controladores/ProductoController.php';
        $controller = new ProductoController();
        $librosLocales = $controller->listar();
        $title_heading = "Libros disponibles en Mi Librería";
...
<nav class="navbar">
    <div class="navbar-container">
        ...
        <div class="navbar-links">
            ...
            <a href="EnTienda.php">🏬 En tienda</a>
...
<div style="display: flex; align-items: center; justify-content: space-between; gap: 10px; margin: 30px 20px 10px;">
    <h1 style="margin: 0; font-size: 26px; color: #2c3e50;"><?= htmlspecialchars($title_heading) ?></h1>
</div>

<?php if ($view === 'authors'): ?>
    <?php
        // Ordenar autores según la opción seleccionada 
        $order = $_GET['order'] ?? 'asc';
        $authors = array_unique($auts);
        natcasesort($authors);
...
<?php elseif ($view === 'tienda'): ?>

    <!-- Mostrar libros locales -->
   

    <div class="book-grid">
    <?php foreach ($librosLocales as $libro): 
        $imagen = !empty($libro['imagen_url']) ? htmlspecialchars($libro['imagen_url']) : 'https://via.placeholder.com/150x200?text=Sin+portada';
    ?>
        <div class="book-card">
...
<?php else: ?>
    <!-- Mostrar libros de la API (por defecto o búsqueda) -->
    <?php if (!empty($error)): ?>
